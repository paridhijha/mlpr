...
minimized_weights = minimize(weights, @log_like_negative, 100, x_train, y_train);
...
% % Q 2.2(a) - label noise model - modify likehihood
% e=0.5; % noise parameter
% %[Lp, dLp_dw, dLp_de] = log_like_noise(weights, x_train, y_train, e);
% %gradW = checkgrad(@log_like_noise, weights, 1e-5, x_train, y_train, 0.5);
% 
% % Q 2.2(b) - label noise model - Fitting a constrained parameter
%  %We can write this parameter as the result of taking the logistic sigmoid of 
%  %an unconstrained parameter a:
% %weights_a = rand(101,1);
% %minimize(weights_a, @log_like_noise_a, 10, x_train, y_train);
