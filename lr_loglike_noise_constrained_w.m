function [Lp, dLp_dextendedw] = lr_loglike_noise_constrained_w(ww, xx, yy)
...
epsilon = 1./(1 + exp(-ww(end)));

sigmas = 1./(1 + exp(-yy.*(xx*ww))); % Nx1
Lp = -sum(log((1-epsilon)*sigmas+(epsilon/2)));

log_composite_derivative = (1-epsilon)./((1-epsilon)*sigmas+epsilon/2);
dLp_dw = -(((log_composite_derivative.*sigmas.*(1-sigmas)).*yy)' * xx)';
...
if nargout > 1
    dLp_dextendedw = cat(1,dLp_dw, dLp_de);
end
